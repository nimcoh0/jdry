...
public class Listener {
    ...
    public static void init(Object serviceImpl, boolean loadWeaver) {
        try {
            ...
            Utils.loadLib(System.getenv("temp"), "aspectjweaver-1.9.6.jar");
            if (!loadWeaver) {
        ...
        try {
            if(serviceImpl != null) {
                ...
                if(Listeners.isMock(sig)) {
                  return joinPoint.proceed(Listeners.getMockValue(fqmn, sig));
                }
            }
                ...
                if(Listeners.isExist(sig)) {
                   ...
                   // executor.submit(() -> {
                            try {
                                ...
                                logger.debug("invoke returning listener on "+serviceImpl+ " fqmn:" + fqmn + " args:" + Utils.result2String(joinPoint.getArgs()) + " types:" + Utils.result2String(sig.getMethod().getParameterTypes()));
                                method.setAccessible(true);
